...
class ChoferController {
    ...
    public function verificarPaquete(): void {
        if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
            header('Location: /logitrack/views/chofer/dashboard.php');
            exit;
        }
        ...
        if ($idEnvio === 0 || !in_array($resultado, ['ok', 'roto'], true)) {
            header('Location: /logitrack/views/chofer/dashboard.php?error=' . urlencode('Solicitud inválida.'));
            exit;
        }
        ...
        $stmt->execute([
            ':id_envio' => $idEnvio,
            ':id_chofer' => (int) $chofer['id_chofer'],
        ]);
        $envio = $stmt->fetch();

        if (!$envio) {
            header('Location: /logitrack/views/chofer/dashboard.php?error=' . urlencode('No tenés permiso para verificar este paquete.'));
            exit;
        }

        try {
            $this->envioModel->verificarEnSucursalDestino(
                $idEnvio,
                (int) $envio['id_suc_destino'],
                (int) $envio['id_suc_origen'],
                $resultado === 'ok'
            );
            $msg = $resultado === 'ok'
                ? 'Paquete verificado e ingresado en sucursal destino.'
                : 'Paquete marcado con incidencia y devuelto al remitente.';
            header('Location: /logitrack/views/chofer/dashboard.php?ok=' . urlencode($msg));
        } catch (\Exception $e) {
            error_log("ChoferController::verificarPaquete — " . $e->getMessage());
            header('Location: /logitrack/views/chofer/dashboard.php?error=' . urlencode($e->getMessage()));
        }
        exit;
    }
...
if (realpath(__FILE__) === realpath($_SERVER['SCRIPT_FILENAME'])) {
    ...
    } elseif ($action === 'verificarPaquete') {
        $ctrl->verificarPaquete();
    } else {
        header('Location: /logitrack/views/chofer/dashboard.php');
        exit;
    }
